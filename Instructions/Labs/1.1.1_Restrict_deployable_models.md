---
lab:
  title: 'AI Ready Governance hands-on exercise: Restrict deployable models with a
    custom Azure initiative'
  description: When using models from Microsoft Foundry and Azure OpenAI with Microsoft
    Foundry, you have the option to implement custom policies to restrict which specific
    models users can deploy. This capability is available for Microsoft Foundry project
    and hub-based project. In addition, you can leverage a built-in policy definition
    to apply similar restrictions to ensure that Azure Machine Learning deployments
    use only approved Registry models. To minimize the number of policy assignments,
    it is possible to combine multiple policy definitions into initiatives.
  duration: 15 minutes
  level: 200
  islab: true
  primarytopics:
